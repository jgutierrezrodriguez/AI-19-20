...
var Neuroevolution = function (options) {
	...
	function randomGaussianNnmber(){
		...
		resultado = Math.sqrt( -2.0 * Math.log( i ) ) 
					* Math.cos( 2.0 * Math.PI * j );
	...
	Generation.prototype.breed = function (g1, g2, nbChilds) {
		...
		for (var nb = 0; nb < nbChilds; nb++) {
			...
			var corte1 = Math.floor((Math.random()* data.network.weights.length -2) +1 );
			var corte2 = Math.floor((Math.random()* data.network.weights.length -1) + corte1 + 1 );
			
			for(var i = corte1; i<corte2;i++){
				data.network.weights[i]=g2.network.weights[i]; i 
			}

			// Perform mutation on some weights.
			for (var i in data.network.weights) {
				if (Math.random() <= self.options.mutationRate) {
					var resultado_mi_funcion = randomGaussianNnmber();
					data.network.weights[i] += resultado_mi_funcion * 
